...
class SemanticFusionModule(nn.Module):
    ...
    def forward(self, v_stage, a_stage):
        ...
        v_weight = torch.sigmoid(self.vselectfusion(v_interact)) # [nbatch*10, stage_num, 1]
        a_weight = torch.sigmoid(self.aselectfusion(a_interact)) # [nbatch*10, stage_num, 1]
        v_interact = v_interact.permute(0, 2, 1).contiguous() # [nbatch*10, 512, stage_num]
        a_interact = a_interact.permute(0, 2, 1).contiguous() # [nbatch*10, 512, stage_num]
        v_out = torch.bmm(v_interact, v_weight).view(-1, 10, self.num_inputs) 
        a_out = torch.bmm(a_interact, a_weight).view(-1, 10, self.num_inputs) # # [nbatch, 10, 512]
...
class MultimodalPyramidAttentionalModule(nn.Module):
    def __init__(self, num_inputs, ffn_dim, num_channels, kernel_size=3, dropout=0.2, nhead=8):
        ...
        self.capture = SemanticCaptureModule(num_inputs, ffn_dim, num_channels, kernel_size, dropout, nhead) # [nbatch, 10, stage_num, 512]
